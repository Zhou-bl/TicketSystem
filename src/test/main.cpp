...
namespace B_PLUS_TREE {
    ...
    struct KeyValue {//键值对
        ...
        KeyValue() : key(""), value(T()){}

        KeyValue(const T &Value) : key("") {
            value = Value;
        }

        KeyValue(const string &Key, const T &Value) {
        ...
        bool operator==(const KeyValue &rhs) const {
            return value == rhs.value && !strcmp(key, rhs.key);
        }
        ...
        bool operator<(const KeyValue &rhs) const {
            string lString = key, rString = rhs.key;
            if (lString == rString) return value < rhs.value;
    ...
    private:
        ...
        KeyValue<T> children[MAX_CHILDREN + 5];
        //存储关键字
    ...
    private:
        void Initialize() {
            ...
            if (!root_index && !head_index && !tail_index) {//空文件,新建根节点和叶子结点;
                Node<T> new_root(0), new_head(1), new_tail(1);
        ...
        void SplitLeaf(Node<T> tmp) {//叶子结点
            Node<T> newLeaf(1);
        ...
        void SplitNonLeaf(Node<T> tmp) {
            ...
            ////newNode.children[MAX_CHILDREN + 1 >> 1] = KeyValue<T>(); //新节点的第一个键值对赋为初始值(最小值);
            newNode.children[0] = KeyValue<T>();
            tree.update(tmp, tmp.index);
            tree.update(newNode, newNode.index);
            KeyValue<T> minKeyValue = FindMin(newNode);
            UpInsert(newNode.fa, minKeyValue, newNode.index);
        }

        void SplitRoot(Node<T> tmp) {
            Node<T> newNode(0), newRoot(0);
            newNode.index = tree.write(newNode);
            ...
            ////newNode.children[MAX_CHILDREN + 1 >> 1] = KeyValue<T>();
            newNode.children[0] = KeyValue<T>();
            KeyValue<T> minKey = FindMin(newNode);
            newRoot.num = 2;
        ...
        void UpInsert(int index, KeyValue<T> newPair, int newIndex) {//在非叶结点中插入新的键值对;
            Node<T> tmp;
            tree.read(tmp, index);
        ...
        void UpChange(int index, KeyValue<T> las, KeyValue<T> cur){
        ...
        bool LeafBorrow(Node<T> tmp){//记得update;
            ...
            if(tmp.pre != -1){
                ...
                if(preNode.num > MIN_CHILDREN){
                    ...
                    preNode.num--; tmp.num++;
                    preNode.children[preNode.num] = KeyValue<T>();
            ...
            if(tmp.next != -1){
                ...
                if(nextNode.num > MIN_CHILDREN){
                    ...
                    nextNode.children[nextNode.num] = KeyValue<T>();
        ...
        bool NoneLeafBorrow(Node<T> tmp){
            Node<T> preNode, nextNode, faNode;
            KeyValue<T> aimPair = FindMin(tmp);
            int prePos, nextPos;
            ...
            if(nextPos < faNode.num){
                ...
                if(nextNode.num > MIN_CHILDREN){
                    ...
                    nextNode.children[0] = KeyValue<T>();
                    nextNode.children[nextNode.num] = KeyValue<T>();
        ...
        void UpDelete(int index, KeyValue<T> aimPair){//在非叶结点中删除关键字;
            if(index == -1){
                cout << "To Root !!!" << endl;
                return;
            }
            ...
            tmp.children[0] = KeyValue<T>();
            tmp.children[tmp.num] = KeyValue<T>();
            tree.update(tmp, tmp.index);
            if(!pos){
        ...
        void MergeNonLeaf(Node<T> tmp){//同样统一向左合并;tmp -> pre, next -> tmp;
            Node<T> preNode, nextNode, faNode;
            KeyValue<T> aimPair = FindMin(tmp);
            tree.read(faNode, tmp.fa);
            ...
            if(prePos != -1){//有左邻居;
                ...
                tree.read(nextNode, faNode.chIndex[nextPos]);
                KeyValue<T> nextMin = FindMin(nextNode);
                tmp.num++;
                tmp.children[tmp.num - 1] = nextMin;
        ...
        void MergeLeaf(Node<T> tmp){//统一向前合并;
            Node<T> preNode, nextNode, nextNextNode;
    ...
    public:
        ...
        void Insert(const string &key, const T &value) {
            ...
            if(tmp.num == 2 && tmp.children[1] == KeyValue<T>()){ //删空之后特判;
                Node<T> ch;
                ...
                if(tmp.index == head_index && tmp.next == tail_index){//head结点特判;
                    ...
                    if(tailNode.num < MIN_CHILDREN){
                        ...
                        for(int i = tailNode.num - 1; i >= chNum; --i){
                            ...
                            tailNode.children[i - chNum] = KeyValue<T>();
                        }
                        for(int i = 0; i < chNum; ++i){
                            tailNode.children[i] = tmp.children[i + tmp.num];
                            tmp.children[i + tmp.num] = KeyValue<T>();
                        }
                        tree.update(tmp, tmp.index);
        ...
        void Delete(const string &key, const T &value){
            ...
            tmp.children[tmp.num] = KeyValue<T>(); //设为空节点;
            tree.update(tmp, tmp.index);
            ...
            if(tmp.num < MIN_CHILDREN && !Check(tmp.fa)){
                ...
                if(!tmp.num && tmp.index == tail_index){//保证2号结点永远不空;
                    ...
                    if(preNode.num){
                        ...
                        tree.update(preNode, preNode.index);
                        tree.update(tmp, tmp.index);
                        UpChange(tmp.fa, KeyValue<T>(), tmp.children[0]);
                    }
                }
            }
            num--;
        ...
        bool check(int index){
            ...
            tree.read(tmp, index);
            if(tmp.is_leaf) return true;
            if(tmp.children[0] != KeyValue<T>()) return false;
            for(int i = 0; i < tmp.num; ++i){
...
int main(){
    ...
    freopen("src/data/tree1.txt","w",stdout);
    BPlusTree<int> tree("src/data/tree1");
    int n, k;
    vector<int> ans;
    string op, s;
    cin >> n;
    for(int i = 1; i <= n; ++i){
        ...
        /*
        if(!tree.check(tree.get_root_index())){
            cout << "children != KeyValue()!" << endl;
        }

        if(!tree.is_zero_non_leaf(tree.get_root_index())){
            cout << "chilren0 is wrong!" << endl;
        }
        if(!tree.is_son_non_fa(tree.get_root_index())){
            cout << "fa is wrong!" << endl;
        }
        ...
        if(!tree.children_num()){
            cout << "children num is wrong!" << endl;
        }
        if(!tree.check_non0_child_is_KeyValue(tree.get_root_index())){
            cout << "check_non0_child_is_KeyValue wrong!" << endl;
        }
         */
    }
    return 0;
